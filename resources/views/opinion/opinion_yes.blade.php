...
@section('content')
    ...
    <div class="page">
        ...
        @if ($questions[2]['checked']=="on")
            <h5 class='subtitle subtitle1' style='position: relative; display: block;'>{{$questions[2]['question']}}
                "<span>*</span> ։<span class='error1'>Պարտադիր է լրացման</span></h5>
        @else
            <h5 class='subtitle subtitle1'
                style='position: relative; display: block;'>{{$questions[2]['question']}}</h5>
        @endif


        @foreach ($answer_2 as  $val)
            @if ($val['type']=='input')
                <input type="{{$val['type']}}" placeholder='Մուտքագրեք տեքստը' class='arr form-control shadow-none'
                       style='border:none;border-bottom: 3px solid #E5E5E6;' id="{{$val['id']}}" name='inp'>

            @else
                @if ($val['type']=='radio')
                    <p><input type="{{$val['type']}}" class='radio' id="{{$val['id']}}" name='inp'><label
                            for="{{$val['id']}}"><span class=span>{{$val['patasxan']}}</span></label></p>
                @else
                    <p><input type="{{$val['type']}}" class='checkbox' id="{{$val['id']}}" name='inp'><label
                            for="{{$val['id']}}"><span class=span>{{$val['patasxan']}}</span></label></p>
                @endif
            @endif
        ...
        @endforeach
        <p><input type="radio" class="other" id="5" name="inp"> <label for="5"><span class=span>Այլ</span></label></p>
        <div class=" letterscount arr1 other-input"><input class="arr form-control shadow-none" id="arr1"
                                                           placeholder="Մուտքագրեք տեքստը" maxlength="50"></input>
     ...
     <form>
        ...
        <div class="next-opinion">
            <button class="btn-primary go "><a class="a">ԱՌԱՋ
                    <svg style="margin-bottom: 2px;" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
            ...
            <a class="prev" href="/opinion"><div class="previous">
